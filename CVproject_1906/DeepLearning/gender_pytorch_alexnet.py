...
def train():
  ...
  # model = models.vgg16(pretrained=True) #加载预训练好的VGG16
   ...
   model.classifier.add_module('fc', nn.Linear(4096, 2)) #相当于修改最后一层的
   print(model)
   model = model.cuda()
   num_params=0
...
#            lr1 = lr1 * 0.9
#            lr2 = lr2 * 0.9
            savepath='face_code/mymodel/model_alexnet_' + str(epoch) +'.pkl'
...
torch.save(trained_model,'face_code/mymodel/model_alexnet_final.pkl')
file=open('face_code/mymodel/alexnet_accuracy.txt','w')
file.write(str(Accuracy_list))
file.close()
file2=open('face_code/mymodel/alexnet_loss.txt','w')
...
plt.show()
plt.savefig("face_code/results/alexnet_train_accuracy_loss_v0.jpg")
