...
def train():
  ...
  # model = models.vgg16(pretrained=True) #加载预训练好的VGG16
   ...
   model.fc = nn.Linear(512, 6) #最后一层直接换成512*2
   print(model)
   model = model.cuda()
   num_params=0
...
#            lr1 = lr1 * 0.9
#            lr2 = lr2 * 0.9
            savepath='face_code/mymodel/age_model_1resnet34_' + str(epoch) +'.pkl'
...
torch.save(trained_model,'face_code/mymodel/age_model_1resnet34_final.pkl')
file=open('face_code/mymodel/age_1resnet34_accuracy.txt','w')
file.write(str(Accuracy_list))
file.close()
file2=open('face_code/mymodel/age_1resnet34_loss.txt','w')
...
plt.show()
plt.savefig("face_code/results/age_1resnet34_train_accuracy_loss_v0.jpg")
